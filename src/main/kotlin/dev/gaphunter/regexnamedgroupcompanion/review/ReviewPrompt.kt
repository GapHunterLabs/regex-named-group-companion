package dev.gaphunter.regexnamedgroupcompanion.review
 ...
 * number of debounced "session of use" events in the tool window --
 * never on install, never on a timer, and never for a raw keystroke
 * (the panel recomputes on every keystroke, but the caller debounces
 * that stream down to one event per real pause after a successful,
 * matching pattern -- see [RegexNamedGroupPanel.scheduleReviewHit]).
 * This is specifically about the tool window, not the separate inline
 * inlay this plugin also has (that's a different CTA family). Counts
...
object ReviewPrompt {
    ...
    private const val KEY_HIT_COUNT = "dev.gaphunter.regexnamedgroupcompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.regexnamedgroupcompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "Regex Named Group Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33786-regex-named-group-companion/reviews"
